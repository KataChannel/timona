...
import { cn, ensureSecureUrl, ensureSecureHtmlContent } from "@/lib/utils";
import {
  beautifyHtml,
  minifyHtml,
  escapeHtml,
  highlightHtml,
} from "@/lib/htmlBeautifier";
...
export function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    extensions: [
      ...
      Image.configure({
        ...
        HTMLAttributes: {
          class:
            "rounded-lg max-w-full h-auto my-4 cursor-pointer hover:ring-2 hover:ring-blue-500 transition-all",
        },
      }),
      Link.configure({
        openOnClick: false,
  ...
  const handleImageSelect = (fileOrUrl: File | string) => {
    ...
    // Ensure secure URL (convert HTTP to HTTPS if on HTTPS page)
  ...
  const handleImageEdit = () => {
    ...
    const attrs: any = {
      src: ensureSecureUrl(imageEditData.src),
  ...
  const applySourceChanges = () => {
    if (!editor) return;
    // Ensure all URLs are secure before applying
    const secureHtml = ensureSecureHtmlContent(sourceHtml);
    // Minify HTML before storing
    const minified = minifyHtml(secureHtml);
          ...
          <div className="p-2 flex flex-wrap items-center gap-1.5">
            ...
            <div className="flex items-center gap-0.5 bg-muted/30 rounded-md p-0.5">
              ...
              <Button
                ...
                className={cn(
                  "h-7 w-7 p-0",
                  editor.isActive({ textAlign: "center" }) && "bg-background",
                )}
                title="Giữa"
              >
              ...
              <Button
                ...
                className={cn(
                  "h-7 w-7 p-0",
                  editor.isActive({ textAlign: "justify" }) && "bg-background",
                )}
                title="Đều"
              >
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      "h-7 w-7 p-0",
                      editor.isActive("table") && "bg-background",
                    )}
                    title="Bảng"
                  >
      ...
      {viewMode === "edit" ? (
        ...
        <ScrollArea className="min-h-[200px] max-h-[600px] p-4">
          <div
            ...
            dangerouslySetInnerHTML={{ __html: ensureSecureHtmlContent(editor?.getHTML() || "") }}
          />
        </ScrollArea>
      )}
      <FilePicker
      ...
      <Dialog open={imageEditOpen} onOpenChange={setImageEditOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Tùy chỉnh kích thước, căn chỉnh và thuộc tính hình ảnh
            </DialogDescription>
          </DialogHeader>
          <ScrollArea className="max-h-[60vh]">
            <div className="space-y-4 p-4">
              <div className="space-y-2">
                <Label htmlFor="img-src">URL hình ảnh</Label>
                <Input
                  id="img-src"
                  value={imageEditData.src}
                  onChange={(e) =>
                    setImageEditData({ ...imageEditData, src: e.target.value })
                  }
                  placeholder="https://example.com/image.jpg"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="img-alt">Alt text (Mô tả)</Label>
                <Input
                  id="img-alt"
                  value={imageEditData.alt}
                  onChange={(e) =>
                    setImageEditData({ ...imageEditData, alt: e.target.value })
                  }
                  placeholder="Mô tả hình ảnh..."
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="img-title">Title (Tiêu đề khi hover)</Label>
                <Input
                  id="img-title"
              ...
              <div className="space-y-2">
                <Label htmlFor="img-width">Chiều rộng (px hoặc %)</Label>
                <Input
                  id="img-width"
                  ...
                  placeholder="VD: 500 hoặc 50%"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="img-align">Căn chỉnh</Label>
                <Select
                  value={imageEditData.align}
                  onValueChange={(value: "left" | "center" | "right") =>
                    setImageEditData({ ...imageEditData, align: value })
                  }
                >
                  <SelectTrigger id="img-align">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setImageEditOpen(false)}>
              Hủy
            </Button>
            <Button onClick={handleImageEdit}>Áp dụng</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>Nhập URL và text hiển thị</DialogDescription>
          </DialogHeader>
          <ScrollArea className="max-h-[60vh]">
            <div className="space-y-4 p-4">
              <div className="space-y-2">
      ...
      <Dialog open={sourceViewOpen} onOpenChange={setSourceViewOpen}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>Xem và chỉnh sửa HTML source</DialogDescription>
          </DialogHeader>
            ...
            <TabsContent value="preview" className="flex-1 mt-4">
              <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: ensureSecureHtmlContent(sourceHtml) }}
                />
              </ScrollArea>
            </TabsContent>
          </Tabs>
          <DialogFooter className="mt-4">
            ...
            <Button onClick={applySourceChanges}>Áp dụng</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
