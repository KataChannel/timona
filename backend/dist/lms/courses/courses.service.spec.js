...
describe('CoursesService', () => {
    ...
    describe('findOne', () => {
        ...
        it('should throw NotFoundException if course not found', async () => {
            ...
            await expect(service.findOne('non-existent')).rejects.toThrow(new common_1.NotFoundException('Course with ID non-existent not found'));
        });
    });
    describe('findBySlug', () => {
        ...
        it('should throw NotFoundException if course not found', async () => {
            mockPrismaService.course.findUnique.mockResolvedValue(null);
            await expect(service.findBySlug('non-existent')).rejects.toThrow(new common_1.NotFoundException('Course with slug non-existent not found'));
        });
    });
    describe('create', () => {
        ...
        it('should generate unique slug if collision occurs', async () => {
            ...
            expect(mockPrismaService.course.create).toHaveBeenCalledWith(expect.objectContaining({
                data: expect.objectContaining({
                    slug: 'duplicate-course-1',
                }),
            }));
        ...
        it('should validate category exists if provided', async () => {
            ...
            await expect(service.create(userId, createInput)).rejects.toThrow(new common_1.BadRequestException('Category not found'));
        });
    });
    describe('update', () => {
        ...
        it('should throw NotFoundException if course not found', async () => {
            mockPrismaService.course.findUnique.mockResolvedValue(null);
            await expect(service.update('1', 'user-1', { id: '1', title: 'New' })).rejects.toThrow(new common_1.NotFoundException('Course with ID 1 not found'));
        });
        it('should throw ForbiddenException if user is not instructor', async () => {
            ...
            mockPrismaService.course.findUnique.mockResolvedValue(mockCourse);
            await expect(service.update('1', 'different-user', { id: '1', title: 'New' })).rejects.toThrow(new common_1.ForbiddenException('You do not have permission to update this course'));
        });
    });
    describe('publish', () => {
        ...
        it('should throw BadRequestException if course has no modules', async () => {
            ...
            mockPrismaService.course.findUnique.mockResolvedValue(mockCourse);
            await expect(service.publish('1', 'user-1')).rejects.toThrow(new common_1.BadRequestException('Course must have at least one module before publishing'));
        });
        it('should throw BadRequestException if course has no lessons', async () => {
            ...
            mockPrismaService.course.findUnique.mockResolvedValue(mockCourse);
            await expect(service.publish('1', 'user-1')).rejects.toThrow(new common_1.BadRequestException('Course must have at least one lesson before publishing'));
        });
        it('should throw ForbiddenException if user is not instructor', async () => {
            const mockCourse = {
                id: '1',
            ...
            mockPrismaService.course.findUnique.mockResolvedValue(mockCourse);
            await expect(service.publish('1', 'different-user')).rejects.toThrow(new common_1.ForbiddenException('You do not have permission to publish this course'));
        });
    });
    describe('archive', () => {
        ...
        it('should throw ForbiddenException if user is not instructor', async () => {
            ...
            mockPrismaService.course.findUnique.mockResolvedValue(mockCourse);
            await expect(service.archive('1', 'different-user')).rejects.toThrow(new common_1.ForbiddenException('You do not have permission to archive this course'));
        });
    });
    describe('remove', () => {
        ...
        it('should throw BadRequestException if course has enrollments', async () => {
            ...
            mockPrismaService.course.findUnique.mockResolvedValue(mockCourse);
            await expect(service.remove('1', 'user-1')).rejects.toThrow(new common_1.BadRequestException('Cannot delete course with active enrollments. Archive it instead.'));
        });
        it('should throw ForbiddenException if user is not instructor', async () => {
            const mockCourse = {
                id: '1',
            ...
            mockPrismaService.course.findUnique.mockResolvedValue(mockCourse);
            await expect(service.remove('1', 'different-user')).rejects.toThrow(new common_1.ForbiddenException('You do not have permission to delete this course'));
        });
    });
