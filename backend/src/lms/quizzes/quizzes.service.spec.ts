...
describe('QuizzesService', () => {
  ...
  describe('createQuiz', () => {
    ...
    it('should throw NotFoundException if lesson not found', async () => {
      ...
      await expect(
        service.createQuiz('user-1', { lessonId: 'non-existent' } as any),
    ...
    it('should throw ForbiddenException if user is not instructor', async () => {
      ...
      await expect(
        service.createQuiz('user-1', { lessonId: 'lesson-1' } as any),
      ).rejects.toThrow(
        new ForbiddenException('Only the course instructor can create quizzes'),
      );
    });
  });
  ...
  describe('getQuiz', () => {
    ...
    it('should throw NotFoundException if quiz not found', async () => {
      ...
      await expect(service.getQuiz('non-existent', 'user-1')).rejects.toThrow(
    ...
    it('should throw ForbiddenException if user is not enrolled', async () => {
      ...
      await expect(service.getQuiz('quiz-1', 'student-1')).rejects.toThrow(
        new ForbiddenException('You must be enrolled in this course to view the quiz'),
      );
    });
  });
  ...
  describe('updateQuiz', () => {
    ...
    it('should throw NotFoundException if quiz not found', async () => {
      ...
      await expect(
        service.updateQuiz('user-1', 'non-existent', {}),
    ...
    it('should throw ForbiddenException if user is not instructor', async () => {
      ...
      await expect(
        ...
        new ForbiddenException('Only the course instructor can update quizzes'),
      );
    });
  });
  ...
  describe('deleteQuiz', () => {
    ...
    it('should throw NotFoundException if quiz not found', async () => {
      ...
      await expect(service.deleteQuiz('user-1', 'non-existent')).rejects.toThrow(
        new NotFoundException('Quiz not found'),
      );
    });
    ...
    it('should throw ForbiddenException if user is not instructor', async () => {
      ...
      await expect(service.deleteQuiz('user-1', 'quiz-1')).rejects.toThrow(
        new ForbiddenException('Only the course instructor can delete quizzes'),
      );
    });
  });
  ...
  describe('submitQuiz', () => {
    ...
    it('should mark as failed if score below passing score', async () => {
      ...
      const submitInput: any = {
        ...
        enrollmentId: 'enrollment-1',
        answers: [
          { questionId: 'question-1', selectedAnswerIds: ['answer-1'] }, // Wrong
        ],
        timeSpent: 10,
      };
    ...
    it('should throw ForbiddenException if enrollment invalid', async () => {
      ...
      await expect(
        service.submitQuiz('user-1', { enrollmentId: 'invalid' } as any),
      ).rejects.toThrow(new ForbiddenException('Invalid enrollment'));
    });
    ...
    it('should throw NotFoundException if quiz not found', async () => {
      ...
      await expect(
        service.submitQuiz('user-1', { enrollmentId: 'enrollment-1', quizId: 'invalid' } as any),
      ).rejects.toThrow(new NotFoundException('Quiz not found'));
    });
  });
  ...
  describe('getQuizAttempt', () => {
    ...
    it('should throw NotFoundException if attempt not found', async () => {
      ...
      await expect(service.getQuizAttempt('user-1', 'non-existent')).rejects.toThrow(
        new NotFoundException('Quiz attempt not found'),
      );
    });
    ...
    it('should throw ForbiddenException if attempt belongs to different user', async () => {
      ...
      await expect(service.getQuizAttempt('user-1', 'attempt-1')).rejects.toThrow(
        new ForbiddenException('You can only view your own quiz attempts'),
      );
    });
  });
});
