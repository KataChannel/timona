...
describe('FilesService', () => {
  ...
  describe('uploadFile', () => {
    const createMockFile = (mimetype: string, size: number) => {
      // Create a buffer with the specified size
      const buffer = Buffer.alloc(size);
      
      // Create a readable stream that emits the full buffer
    ...
    it('should upload an image file successfully', async () => {
      const mockFile = createMockFile('image/jpeg', 1024 * 1024); // 1MB
      const expectedUrl = 'https://minio.example.com/uploads/test-file.jpg';

      mockMinioService.uploadFile.mockResolvedValue(expectedUrl);

      const result = await service.uploadFile(mockFile as any, 'user-1', 'image');
    ...
    it('should upload a video file successfully', async () => {
      const mockFile = createMockFile('video/mp4', 50 * 1024 * 1024); // 50MB
      const expectedUrl = 'https://minio.example.com/uploads/video.mp4';

      mockMinioService.uploadFile.mockResolvedValue(expectedUrl);

      const result = await service.uploadFile(mockFile as any, 'user-1', 'video');
    ...
    it('should upload a document file successfully', async () => {
      const mockFile = createMockFile('application/pdf', 2 * 1024 * 1024); // 2MB
      const expectedUrl = 'https://minio.example.com/uploads/doc.pdf';

      mockMinioService.uploadFile.mockResolvedValue(expectedUrl);

      const result = await service.uploadFile(mockFile as any, 'user-1', 'document');
    ...
    it('should throw BadRequestException for invalid image type', async () => {
      ...
      await expect(service.uploadFile(mockFile as any, 'user-1', 'image')).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.uploadFile(mockFile as any, 'user-1', 'image')).rejects.toThrow(
        /Invalid file type/,
    ...
    it('should throw BadRequestException for invalid video type', async () => {
      ...
      await expect(service.uploadFile(mockFile as any, 'user-1', 'video')).rejects.toThrow(
        BadRequestException,
      );
    });

    it('should throw BadRequestException if video file size exceeds limit', async () => {
      const mockFile = createMockFile('video/mp4', 600 * 1024 * 1024); // 600MB > 500MB limit
    ...
    it('should throw BadRequestException if document file size exceeds limit', async () => {
      const mockFile = createMockFile('application/pdf', 15 * 1024 * 1024); // 15MB > 10MB limit

      await expect(service.uploadFile(mockFile as any, 'user-1', 'document')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('uploadLessonVideo', () => {
    ...
    it('should upload video for authorized instructor', async () => {
      ...
      const result = await service.uploadLessonVideo(mockFile as any, 'user-1', 'course-1');
    ...
    it('should throw BadRequestException if course not found', async () => {
      ...
      await expect(
        service.uploadLessonVideo(mockFile as any, 'user-1', 'non-existent'),
      ).rejects.toThrow(
        new BadRequestException('Not authorized to upload to this course'),
      );
    });
    ...
    it('should throw BadRequestException if user is not instructor', async () => {
      ...
      await expect(
        service.uploadLessonVideo(mockFile as any, 'user-1', 'course-1'),
      ).rejects.toThrow(
        new BadRequestException('Not authorized to upload to this course'),
      );
    });
  ...
  describe('uploadCourseThumbnail', () => {
    ...
    it('should upload thumbnail without courseId', async () => {
      ...
      const result = await service.uploadCourseThumbnail(mockFile as any, 'user-1');
    ...
    it('should upload thumbnail with courseId and authorization check', async () => {
      ...
      const mockFile = createMockImageFile();
      const result = await service.uploadCourseThumbnail(mockFile as any, 'user-1', 'course-1');
    ...
    it('should throw BadRequestException if user is not authorized', async () => {
      ...
      await expect(
        service.uploadCourseThumbnail(mockFile as any, 'user-1', 'course-1'),
      ).rejects.toThrow(new BadRequestException('Not authorized'));
    });
  });
  ...
  describe('uploadCourseMaterial', () => {
    ...
    it('should upload course material for authorized instructor', async () => {
      ...
      const result = await service.uploadCourseMaterial(mockFile as any, 'user-1', 'course-1');
    ...
    it('should throw BadRequestException if not authorized', async () => {
      ...
      await expect(
        service.uploadCourseMaterial(mockFile as any, 'user-1', 'course-1'),
      ).rejects.toThrow(new BadRequestException('Not authorized'));
    });
  });
  ...
  describe('deleteFile', () => {
    ...
    it('should throw BadRequestException on delete error', async () => {
      ...
      await expect(service.deleteFile('file-123', 'uploads', 'user-1')).rejects.toThrow(
  ...
  describe('getPresignedUrl', () => {
    ...
    it('should throw BadRequestException on presigned URL error', async () => {
      ...
      await expect(service.getPresignedUrl('file-123', 'uploads')).rejects.toThrow(
        new BadRequestException('Failed to generate presigned URL'),
      );
    });
  });
});
